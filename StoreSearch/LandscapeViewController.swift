...
//            button.setTitle("\(index)", for: .normal)
            button.frame = CGRect(
                x: x + paddingHorz,
                y: marginY + CGFloat(row)*itemWidth + paddingVert,
                width: buttonWidth,
                height: buttonHeight)
    ...
    private func downloadImage(for searchResult: SearchResult, andPlaceOn button: UIButton){
        if let url = URL(string: searchResult.imageSmall){
            let task = URLSession.shared.downloadTask(
                with: url) { [weak button] url, _, error in
                    if error == nil,
                       let url = url,
    ...
    private func showNothingFoundLabel(){
        ...
        var rect = label.frame.size
        rect.width = ceil(rect.width / 2) * 2       // make even
        rect.height = ceil(rect.height / 2) * 2
